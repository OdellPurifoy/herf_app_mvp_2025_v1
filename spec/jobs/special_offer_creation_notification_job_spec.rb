...
RSpec.describe SpecialOfferCreationNotificationJob, type: :job do
  describe '#perform' do
    ...
    it 'enqueues the job' do
      expect do
        SpecialOfferCreationNotificationJob.perform_later(special_offer.id)
      end.to have_enqueued_job(SpecialOfferCreationNotificationJob).with(special_offer.id)
    end
    ...
    it 'sends notifications to active members who allow text notifications and have not opted out' do
      ...
      SpecialOfferCreationNotificationJob.perform_now(special_offer.id)
    end
    ...
    it 'returns message when special offer is not found' do
      ...
      expect(SpecialOfferCreationNotificationJob.perform_now(999)).to be_nil
    end
    ...
    it 'includes special offer information in the notification message' do
      expect(SmsNotificationService).to receive(:new).with(
        to: member1.phone_number,
        body: /new special offer/i
      ).and_return(sms_service)
