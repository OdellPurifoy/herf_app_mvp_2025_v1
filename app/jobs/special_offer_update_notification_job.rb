...
class SpecialOfferUpdateNotificationJob < ApplicationJob
  queue_as :default
  ...
  def perform(special_offer_id)
    ...
    members.each do |member|
      ...
      SmsNotificationService.new(to: member.phone_number, body: update_message_for(special_offer, member)).send_message
    end
  end

  private

  def update_message_for(special_offer, member)
    <<~SMS
      Hi #{member.first_name},
      #{special_offer.lounge.name} has updated a special offer for you!
      "#{special_offer.name}"
